...
def zixun_index(request):
    ...
    curson_zixun_pinglun.execute("select  pl.riqi,pl.neirong,xw.id  from xinwen_pinglun as pl,xinwen  as xw  where pl.yn_shenhe=2 and  pl.zixun_id = xw.id order by id desc limit 3 ")
    ...
    for zixun_fenlei in rows_zixun_fenlei: #循环每个分类id
        ...
        curson_zixun_zuixin.execute(
            "select id,xinxi_lxid1,xinxi_biaoti,xinxi_riqi from xinwen where xinxi_lxid1=%s order by id desc limit 5" % zixun_fenlei[0])
...
def api_zixun_shoucang(request):
    ...
    if info:
        ...
        sql_insert = "insert into xinwen_shoucang(zixun_id,u_id,riqi,shijian) values ('%s','%s','%s','%s')"  % (zixun_id,u_id,add_riqi,add_shijian)
...
def api_zixun_pinglun_add(request):
    ...
    if info:
        ...
        add_shijian = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
        curson_insert = connection.cursor()
        sql_insert = "insert into xinwen_pinglun(neirong,u_id,zixun_id,riqi,shijian) values ('%s',%s,%s,'%s','%s')" % (neirong, h_id, zixun_id,add_riqi, add_shijian)
        curson_insert.execute(sql_insert)
...
def mem_zixun_shoucang_list(request, dijiye):
    ...
    sql = "select  sc.id as scid,sc.riqi,xw.id as xwid,xw.xinxi_biaoti from xinwen_shoucang as sc,xinwen as xw " \
          "where sc.zixun_id=xw.id and sc.u_id=%s  order by sc.id desc limit %s,%s" % (h_id,int(meiye) * int(dijiye), meiye)
    print(sql)
    cursor.execute(sql)
    ...
    for row in rows:
        ...
        biaoge = biaoge + '<td bgcolor="#FFFFFF"><a href="/xinxi_xiangqing?zxid=%s" target="_blank">%s</a></td>' % ( row[2], row[3]) #会员信息
...
def mem_zixun_pinglun_list(request, dijiye):
    ...
    sql = "select  pinglun.id as plid,pinglun.riqi,pinglun.neirong,pinglun.yn_shenhe,xw.id as xwid,xw.xinxi_biaoti from xinwen_pinglun as pinglun,xinwen as xw " \
          "where pinglun.zixun_id=xw.id and pinglun.u_id=%s  order by pinglun.id desc limit %s,%s" % (h_id,int(meiye) * int(dijiye), meiye)
    print(sql)
    cursor.execute(sql)
    rows = cursor.fetchall()  # 获取所有的数据
    ...
    for row in rows:
        biaoge = biaoge + '<tr>'
        biaoge = biaoge + '<td bgcolor="#FFFFFF" height="27">%s</td>' %  row[1] #评论时间
        biaoge = biaoge + '<td bgcolor="#FFFFFF"><a href="/xinxi_xiangqing?zxid=%s" target="_blank">%s</a></td>' % ( row[4], row[5]) #会员信息
...
def mem_zixun_liulan_list(request, dijiye):
    ...
    sql = "select  liulan.id as llid,liulan.riqi,xw.id as xwid,xw.xinxi_biaoti from xinwen_liulan as liulan,xinwen as xw " \
          "where liulan.zixun_id=xw.id and liulan.u_id=%s  order by liulan.id desc limit %s,%s" % (h_id,int(meiye) * int(dijiye), meiye)
    print(sql)
    cursor.execute(sql)
    rows = cursor.fetchall()  # 获取所有的数据
...
def xinxi_xiangqing(request):
    if request.method == "GET":
        ...
        if h_id is None:
            curson_insert = connection.cursor()
            sql_insert = "insert into xinwen_liulan(zixun_id,riqi,shijian) values ('%s','%s','%s')" % (zxid, add_riqi, add_shijian)
            curson_insert.execute(sql_insert)
        else:
            curson_insert = connection.cursor()
            sql_insert = "insert into xinwen_liulan(zixun_id,u_id,riqi,shijian) values ('%s','%s','%s','%s')" % (zxid, h_id, add_riqi, add_shijian)
            curson_insert.execute(sql_insert)
        ...
        curson_pinglun_list.execute("select id,neirong,riqi  from xinwen_pinglun where yn_shenhe=2 and zixun_id=%s" % zxid)
...
def xinxi_list(request,dijiye,leixing_id):
    ...
    if leixing_id == "0":  # 字符串 非整数，后面要转
        if chaxun is None:
            ...
            sql = "select * from xinwen where xinxi_biaoti like '%%%s%%'  order by id desc limit %s,%s" % (chaxun,(int(meiye) * int(dijiye)), meiye)
    else:
        sql = "select * from xinwen where xinxi_lxid1=%s order by id desc limit %s,%s" % (leixing_id, int(meiye) * int(dijiye), meiye)
    ...
    if int(dijiye) >= 1:
        caidan = caidan + '<a href="/xinxi_list/%s/%s?remen=%s">上一页</a>&nbsp;&nbsp;' % ((int(dijiye) - 1),leixing_id,chaxun)
    else:
        caidan = caidan + '上一页&nbsp;&nbsp;'
    ...
    if int(dijiye) >= (int(yeshu) - 1):
        ...
        caidan = caidan + '<a href="/xinxi_list/%s/%s?remen=%s">下一页</a>&nbsp;&nbsp;' % ((int(dijiye) + 1),leixing_id,chaxun)
