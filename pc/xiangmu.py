...
def pc_xiangmu_xiangqing(request):
    if request.method == "GET":
        ...
        curson_mulu1.execute(
            "select  id,xinxi_biaoti,up_id  from xiangmu_mulu where mulu_jibie=1 and xiangmu_id=%s" % cpid)
        rows_mulu1 = curson_mulu1.fetchall()

        # 读取2级目录
        curson_mulu2 = connection.cursor()
        curson_mulu2.execute(
            "select  id,xinxi_biaoti,up_id  from xiangmu_mulu where mulu_jibie=2 and xiangmu_id=%s" % cpid)
        ...
        curson_pinglun_list.execute(
            "select id,neirong,riqi  from xiangmu_pinglun where yn_shenhe=2 and xiangmu_id=%s" % cpid)
...
def api_xiangmu_pinglun_add(request):
    ...
    cuowu = ""
    if info:
        cuowu = "<script>alert('您已经提交过评论');window.parent.location='/pc_xiangmu_xiangqing?id=" + str(
            xiangmu_id) + "';</script>"
    else:
        # #写入数据库
        add_riqi = time.strftime("%Y-%m-%d", time.localtime())
