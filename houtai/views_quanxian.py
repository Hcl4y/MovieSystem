...
def ht_caidan_1ji(request):
    ...
    if request.method == "POST":
        ...
        if id_1ji:
            ...
            curson = connection.cursor()
            sql = "update quanxian_caidan set caidan_mingcheng='%s',paixu_id=%s where id=%s" % (
...
def ht_caidan_2ji_iframe(request):
    ...
    if request.method == "POST":
        ...
        if id_2ji:  # 更新2级菜单  程序
            ...
            curson = connection.cursor()
            sql = "update quanxian_caidan set caidan_mingcheng='%s',paixu_id=%s where id=%s" % (
            caidan_mingcheng, paixu_id, id_2ji)
            curson.execute(sql)
        else:  # 录入2级菜单  程序
            caidan_mingcheng = request.POST.get("caidan_mingcheng")
            paixu_id = request.POST.get("paixu_id")
...
def ht_caidan_3ji(request):
    if request.method == "GET":
        ...
        if id_1ji:
            curson = connection.cursor()
            curson.execute(
                "select id,caidan_mingcheng from quanxian_caidan where caidan_jibie=2 and caidan_suoshu=%s" % id_1ji)
...
def ht_caidan_3ji_iframe(request):
    ...
    if request.method == "POST":
        ...
        if id_3ji:  # 更新3级菜单  程序
            ...
            paixu_id = request.POST.get("paixu_id")
            curson = connection.cursor()
            sql = "update quanxian_caidan set caidan_mingcheng='%s',caidan_lujing='%s',paixu_id=%s where id=%s" % (
            caidan_mingcheng, caidan_lujing, paixu_id, id_3ji)
            curson.execute(sql)
        else:  # 录入3级菜单  程序
            caidan_mingcheng = request.POST.get("caidan_mingcheng")
            caidan_lujing = request.POST.get("caidan_lujing")
            ...
            for row2 in list2:
                ...
                for row3 in list3:
                    ...
                    if tmp in fenzu_quanxian3:
                        caidan = caidan + "<input type='checkbox' name='quanxian' value='%s' checked  />" % row3[0] + \
                                 row3[1] + " "
                    else:
                        caidan = caidan + "<input type='checkbox' name='quanxian' value='%s' />" % row3[0] + row3[
                            1] + " "
