...
def ht(request):
    ...
    if request.method =="POST":
        ...
        curson = connection.cursor()
        curson.execute("select * from quanxian_yonghu where user_name='%s' and user_password='%s' " % (user_name,user_password))
...
def ht_left2(request):
    ...
    sql_2ji = "select * from quanxian_caidan where caidan_jibie=2 and caidan_suoshu=%s  and id in(0%s0)" % (id_1ji, request.COOKIES.get("quanxian2"))
    ...
    if list2:
        for row2 in list2:
            ...
            sql_3ji = "select * from quanxian_caidan where caidan_jibie=3 and caidan_suoshu=%s and id in(0%s0)" % ( row2[0], request.COOKIES.get("quanxian3"))
